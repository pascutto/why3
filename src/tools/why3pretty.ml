...
module LatexInd (Cnf: sig val prefix: string val flatten_applies : bool end) = struct
  ...
  let main fmt mlw_file paths =
      ...
      Requirements.iter (pp_command_shape ~comment_macros:true fmt) !requirements;
    ...
    | Some filename ->
        ...
        (match !output, !kind with
         ...
         | Some Latex, Some Inductive ->
             ...
             let module M = LatexInd(struct let prefix = !prefix let flatten_applies = true end) in
