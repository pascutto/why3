...
let elim le_int le_real neg_real type_kept kn
  ...
  | Dtype ts when Mts.exists (fun ts' _ -> ts_equal ts ts') float_metas
      ...
      let e_val = Number.(ConstReal (RConstVal (real_value m ~pow2:e ?pow5:None))) in
...
let rec replace_negative_constants neg_int neg_real t =
  ...
  | Some ty, Tconst (ConstInt (IConstVal v) as c) ->
     if BigInt.lt v BigInt.zero && ty_equal ty ty_int then
       t_app neg_int [t_const (neg c) ty_int] (Some ty_int)
     else t
  | Some ty, Tconst (ConstInt (IConstLit i) as c) ->
     if i.il_neg && ty_equal ty ty_int then
       t_app neg_int [t_const (neg c) ty_int] (Some ty_int)
     else t
  | Some ty, Tconst (ConstReal (RConstVal r) as c) ->
     if BigInt.lt r.rv_sig BigInt.zero && ty_equal ty ty_real then
       t_app neg_real [t_const (neg c) ty_real] (Some ty_real)
     else t
  | Some ty, Tconst (ConstReal (RConstLit r) as c) ->
     if r.rl_neg && ty_equal ty ty_real then
       t_app neg_real [t_const (neg c) ty_real] (Some ty_real)
     else t
