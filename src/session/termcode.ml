...
let const_shape c =
  match c with
  | ConstInt (IConstVal c) ->
      push (BigInt.to_string c)
  | ConstInt (IConstLit { il_kind=k; il_neg=n; il_int=i }) ->
      sign_shape n; integer_kind_shape k; push i
  | ConstReal (RConstVal { rv_sig=i; rv_pow2=p2; rv_pow5=p5 }) ->
      pushb i; pushc 'p'; pushb p2; pushc 'e'; pushb p5
  | ConstReal (RConstLit { rl_kind=k; rl_neg=n; rl_int=i; rl_frac=f; rl_exp=e }) ->
      sign_shape n; real_kind_shape k;
      push i; pushc '.'; push f;
      match e,k with
      | Some e, RLitDec -> pushc 'e'; push e
      | Some e, RLitHex -> pushc 'p'; push e
      | None, _ -> ()
...
module Checksum = struct
  ...
  let const b c =
    ...
    | ConstInt (IConstVal c) ->
        raw_bigint b c
    | ConstInt (IConstLit { il_kind=k; il_neg=n; il_int=i }) ->
        sign b n; integer_kind b k; raw_string b i
    | ConstReal (RConstVal { rv_sig=i; rv_pow2=p2; rv_pow5=p5 }) ->
        raw_bigint b i; char b 'p'; raw_bigint b p2; char b 'e'; raw_bigint b p5
    | ConstReal (RConstLit { rl_kind=k; rl_neg=n; rl_int=i; rl_frac=f; rl_exp=e }) ->
        sign b n; real_kind b k;
        raw_string b i; char b '.'; raw_string b f;
        match e,k with
        | Some e, RLitDec -> char b 'e'; raw_string b e
        | Some e, RLitHex -> char b 'p'; raw_string b e
        | None, _ -> ()
