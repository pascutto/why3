...
let t_int_const s =
  t_const (Number.(ConstInt (IConstLit (int_literal ILitDec ~neg:false s)))) ty_int
...
let rec term denv env impl { e_loc = loc; e_node = n } = match n with
  ...
  | Enum (Nreal (i,f,e)) ->
      t_const (Number.(ConstReal (RConstLit (real_literal RLitDec ~neg:false ~int:i ~frac:(Opt.get_def "0" f) ~exp:e)))) ty_real
  | Enum (Nrat (n,d)) ->
